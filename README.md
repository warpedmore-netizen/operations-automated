...
>
> **Current boundary:** Commercial validation, external publication, application deployment and any connection beyond the privately validated Confluence route require separate decisions. The governed lifecycle-first Confluence mirror is approved for private internal validation. A human-first reader layer is proposed separately; no migration, automatic publication or external publication is approved.
>
...
The first live publication proved the controls and created 108 reconciled pages. Founder review then identified that the result remains a controlled repository mirror rather than a human publication. The proposed [Confluence human publication model](product/confluence-human-publication-model.md) adds a curated reader layer for a book-like methodology and organisational policies, frameworks, functionality documentation and user guides, while retaining the lifecycle mirror as a secondary controlled record. It also proposes that authorised document edits, comments and feedback enter the governed methodology challenge loop; automatic retrieval remains separately unapproved.
