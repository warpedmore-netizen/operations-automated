...
- No live credentials, browseable pickers, recurring sync, AI calls or publication.
...
1. Add tenant, membership, connector-installation, selection-rule, source-object and snapshot tables.
2. Replace service-owner static tokens with tenant-scoped OAuth installations.
3. Implement the Confluence space/page-tree browser and selection-rule API.
4. Run read-only sync into source snapshots and inventory.
5. Add provenance-backed extraction candidates and question generation.
6. Add the managed AI example behind explicit consent, quotas and fictional-data guidance.
7. Test end to end with fictional companies of different maturity.
8. Add Google Drive using the same connector and picker contracts.
9. Add BYO AI after secret lifecycle, policy and deletion controls are complete.
