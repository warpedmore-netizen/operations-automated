...
test("answers and feedback actions lead with plain-language outcomes", () => {
  ...
  assert.match(appSource, /saves a change candidate and opens its review automatically/i);
  assert.match(appSource, /What happens when you choose/);
  assert.match(appSource, /Save this use/);
  assert.match(appSource, /A change review is created or opened automatically/);
  ...
  assert.match(html, /change candidate opens its separate change review automatically/);
