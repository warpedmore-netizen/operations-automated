...
async function panel() {
  ...
  section.innerHTML = `<h2>Connected draft assembly</h2>${!allQuestionsAnswered?'<p>Answer the six required governance questions to generate candidate operational objects.</p>':!candidates.length?`<form class="decision" id="generate-guided"><input type="hidden" name="intakeId" value="${intake.id}"><label>Operator<input name="actor" required value="Jamie Peppard"></label><div class="wide"><button class="action">Generate candidate objects</button></div></form>`:!allCandidatesReviewed?'<p>Accept, amend or reject every candidate above before assembling the draft.</p>':!draft?`<form class="decision" id="assemble-draft"><input type="hidden" name="intakeId" value="${intake.id}"><label>Draft builder<input name="actor" required value="Jamie Peppard"></label><div class="wide"><button class="action">Build connected draft</button></div></form>`:`<div class="success">Connected draft assembled from accepted candidates. It remains unapproved.</div><pre>${escapeHtml(draft.preview)}</pre>`}`;
