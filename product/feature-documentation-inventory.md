...
| F16 | Scheduled AI-owner queue and evidence return | `product/operate-internal-workbench.md`, `feedback/2026-07-26-ai-owned-work-needs-a-worker.md`; Jamie and the Codex worker | Implemented proposal with live local automation, **proposed** | Queue/claim/return contract, UI fallback, recurring worker instructions and tests | Availability/catch-up monitoring, stale-claim recovery, service authentication and multi-worker policy | Jamie approves product meaning; task-specific human gates remain unchanged |
